...
$qiandaodb_in_miao = DB::fetch_first("SELECT * FROM ".DB::table('wishing_hall_miao')." WHERE uid='$_G[uid]'");
$stats = DB::fetch_first("SELECT * FROM ".DB::table('wishing_hallset')." WHERE id='5'");
...
$emots = unserialize($_G['setting']['paulsign_emot_miao']);
...
$firstvister = DB::fetch_first("SELECT m.username,q.todaysay,q.godsay FROM ".DB::table('wishing_hall_wish_miao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid and time >= {$tdtime} ORDER BY q.time");
	if($firstvister){
		$firstvister="<li><span class='xi2 xg1'>".$firstvister['username']."</li><li style=' text-align:left;'><span style='color:#F00;'>心愿：</span>".$firstvister['todaysay']."</li><li style=' text-align:left;'><span style='color:#F00;'>喵神答道：</span>".$firstvister['godsay']."</li>";	
	}else{
	 	$firstvister="<li><span class='xi2 xg1'>今天还没有人许愿</li>";	
	}	

$goodpeople = DB::fetch_first("SELECT m.username,q.todaysay,q.godsay FROM ".DB::table('wishing_hall_wish_miao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid and qdxq = 'shuai_miao' ORDER BY q.time desc");
	if($goodpeople){
		$goodpeople="<li><span class='xi2 xg1'>".$goodpeople['username']."</li><li style=' text-align:left;'><span style='color:#F00;'>心愿：</span>".$goodpeople['todaysay']."</li><li style=' text-align:left;'><span style='color:#F00;'>喵神答道：</span>".$goodpeople['godsay']."</li>";	
	}else{
	 	$goodpeople="<li><span class='xi2 xg1'>本庙久经风雨，几近坍塌。</li>";	
	}
		
$master=DB::fetch_first("SELECT m.username,q.todaysay,q.godsay FROM ".DB::table('wishing_hall_miao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY reward desc");	
	if($master){
		$master="<li><span class='xi2 xg1'>".$master['username']."</li><li style=' text-align:left;'><span style='color:#F00;'>心愿：</span>".$master['todaysay']."</li><li style=' text-align:left;'><span style='color:#F00;'>喵神答道：</span>".$master['godsay']."</li>";	
	}else{
	 	$master="<li><span class='xi2 xg1'>本庙暂无堂主</li>";	
	}
	
$sql_rank="SELECT m.username,q.reward,q.uid FROM ".DB::table('wishing_hall_miao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY reward desc LIMIT 0,8";	
...
***********6 E*********/		
//		$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('wishing_hall_miao')."");
//		$page = max(1, intval($_GET['page']));
//		$start_limit = ($page - 1) * 10;
//		$multipage = multi($num, 10, $page, "plugin.php?id=wishing_hall:miao&operation={$_GET[operation]}");
//	}
/*****5 S*******
...
*/			
			$sql = "SELECT q.time,q.qdxq,q.uid,q.todaysay,q.godsay,m.username FROM ".DB::table('wishing_hall_wish_miao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY q.time desc LIMIT 0, 10";
/*		}
...
*******5 E******/
	...
	while($mrc = DB::fetch($query)) {
		...
		if($mrc['qdxq']=="kx_miao"){$mrc['qdxq']="虔诚的想要听取神的教诲，并说";
		}elseif($mrc['qdxq']=="ng_miao"){$mrc['qdxq']="给神磕了个大大的响头，并说";
		}elseif($mrc['qdxq']=="ym_miao"){$mrc['qdxq']="给神上了一炷香，并说";
		}elseif($mrc['qdxq']=="wl_miao"){$mrc['qdxq']="给神献上鲜花，并说";
		}elseif($mrc['qdxq']=="nu_miao"){$mrc['qdxq']="给神敬上一个摩提，并说";
		}elseif($mrc['qdxq']=="ch_miao"){$mrc['qdxq']="给神敬上一杯酒，并说";
		}elseif($mrc['qdxq']=="yl_miao"){$mrc['qdxq']="给神献上巧克力，并说";
		}elseif($mrc['qdxq']=="shuai_miao"){$mrc['qdxq']="要给考神修庙建祠堂，并说";
		}
		$mrcs[] = $mrc;
		$has_items=1;
	}
	...
	$emotquery = DB::query("SELECT count,name FROM ".DB::table('wishing_hallemot')."  WHERE god_id='5' ORDER BY count desc LIMIT 0, 5");
	$emottops = array();
	...
	if($_G['adminid'] == 1) {
		DB::query("UPDATE ".DB::table('wishing_hall_wish_miao')." SET todaysay='{$lang['ban_01']}' WHERE uid='".intval($_GET['banuid'])."'");
		DB::query("UPDATE ".DB::table('wishing_hall_miao')." SET todaysay='{$lang['ban_01']}' WHERE uid='".intval($_GET['banuid'])."'");
...
/*************3 E******************/	
	...
	if($var['lockopen']){
		while(discuz_process::islocked('wishing_hall_miao', 5)){
			usleep(100000);
		}
	}
...
*/	
	...
	$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('wishing_hall_wish_miao')." WHERE time >= {$tdtime}");
	if(!$qiandaodb['uid']) {
		DB::query("INSERT INTO ".DB::table('wishing_hall')." (uid,time) VALUES ('$_G[uid]',$_G[timestamp])");
		DB::query("INSERT INTO ".DB::table('wishing_hall_miao')." (uid,reward) VALUES ('$_G[uid]','0')");
	}elseif(!$qiandaodb_in_miao['uid']){
		DB::query("INSERT INTO ".DB::table('wishing_hall_miao')." (uid,reward) VALUES ('$_G[uid]','0')");
		}
/************1 S*******************	
		if($_GET[qdxq]=='kx'){
...
*********************************/	
	$credit=DB::result_first("SELECT price FROM ".DB::table('wishing_hallemot')." WHERE qdxq='$_GET[qdxq]' and god_id=5");
...
*************************/	
	...
	updatemembercount($_G['uid'], array( "extcredits5"=> $random_rewards));
		
		DB::query("UPDATE ".DB::table('wishing_hall')." SET days=days+1,mdays=mdays+1,time='$_G[timestamp]',qdxq='$_GET[qdxq]',todaysay='$todaysay',godsay='$fastreplytexts[$godword]',flow_up=flow_up+{$credit},chip_award=chip_award+{$random_rewards},lastreward='$credit',lasted='1' WHERE uid='$_G[uid]'");
DB::query("UPDATE ".DB::table('wishing_hall_miao')." SET todaysay='$todaysay',godsay='$fastreplytexts[$godword]',reward=reward+{$credit} WHERE uid='$_G[uid]'");		
//	}
/*************2 S******************/
DB::query("INSERT INTO ".DB::table('wishing_hall_wish_miao')." (uid,time,qdxq,todaysay,godsay) VALUES ('$_G[uid]',$_G[timestamp],'$_GET[qdxq]','$todaysay','$fastreplytexts[$godword]')");
...
/*************2 E******************/	
	...
	if($num <= ($extreward_num - 1) ) {
		...
		if($var['qdtype'] == '2') {
			...
			if($num=='0' || $stats['qdtidnumber'] == '0') {
				$subject=str_replace(array('{m}','{d}','{y}','{bbname}','{author}','{godname}'),array(dgmdate($_G['timestamp'], 'n',$var['tos']),dgmdate($_G['timestamp'], 'j',$var['tos']),dgmdate($_G['timestamp'], 'Y',$var['tos']),$_G['setting']['bbname'],$_G['username'],"喵神堂"),$var['title_thread']);
				$hft = dgmdate($_G['timestamp'], 'Y-m-d H:i',$var['tos']);
				if($exacr && $exacz) {
					$message = "[quote][size=2][color=dimgray]{$lang[tsn_10]}[/color][url={$_G[siteurl]}plugin.php?id=wishing_hall:miao][color=darkorange]{$lang[tsn_11]}[/color][/url][color=dimgray]{$lang[tsn_12]}[/color][/size][/quote][quote][size=2][color=gray][color=teal] [/color][color=gray]{$lang[tsn_01]}[/color] [color=darkorange]{$hft}[/color] {$lang[tsn_02]}[color=red]{$lang[tsn_03]}[/color][color=darkorange]{$lang[tsn_04]}{$lang[tsn_13]}{$lang[tsn_05]}[/color]{$lang[tsn_06]} [/color][color=gray]{$_G[setting][extcredits][$var[nrcredit]][title]} [/color][color=darkorange]{$credit}[/color][color=gray]{$_G[setting][extcredits][$var[nrcredit]][unit]}[/color][color=gray]{$lang[tsn_17]}[/color] [color=gray]{$_G[setting][extcredits][$exacr][title]} [/color][color=darkorange]{$exacz}[/color][color=gray]{$_G[setting][extcredits][$exacr][unit]}.{$another_vip}[/color][/color][/size][/quote][size=3][color=dimgray]{$lang[tsn_07]}[color=red]{$todaysay}[/color]{$lang[tsn_08]}[/color][/size]";
				} else {
				...
				$tid = DB::insert_id();
				DB::query("UPDATE ".DB::table('wishing_hallset')." SET qdtidnumber = '$tid' WHERE id='5'");
	...
	if($num ==0) {
		if($stats['todayq'] > $stats['highestq']) DB::query("UPDATE ".DB::table('wishing_hallset')." SET highestq='$stats[todayq]' WHERE id='5'");
		DB::query("UPDATE ".DB::table('wishing_hallset')." SET yesterdayq='$stats[todayq]',todayq=1 WHERE id='5'");
		DB::query("UPDATE ".DB::table('wishing_hallemot')." SET count=0 WHERE god_id='5'");
		DB::query("DELETE from ".DB::table('wishing_hall_wish_miao')." where time<{$tdtime}-3600*24");//删除前天及以前的记录
	} else {
		DB::query("UPDATE ".DB::table('wishing_hallset')." SET todayq=todayq+1 WHERE id='5'");
	}
	...
	if($var['lockopen']) discuz_process::unlock('wishing_hall_miao');
/************************	
	if($var['tzopen']) {
		if($exacr && $exacz) {
...
*************************/		
		if($exacr && $exacz) {
			sign_msg("{$lang[tsn_14]}{$lang[tsn_03]}{$lang[tsn_04]}{$psc}{$lang[tsn_15]}{$lang[tsn_06]} 上传量 {$credit} GB  {$_G[setting][extcredits][$var[nrcredit]][unit]} {$lang[tsn_16]} 筹码 {$random_rewards} {$_G[setting][extcredits][$exacr][title]} {$exacz} {$_G[setting][extcredits][$exacr][unit]}.".$another_vip,"plugin.php?id=wishing_hall:miao");
		} else {
			sign_msg("{$lang[tsn_18]} 筹码 {$random_rewards} {$_G[setting][extcredits][$var[nrcredit]][title]} {$credit} {$_G[setting][extcredits][$var[nrcredit]][unit]}.".$another_vip,"plugin.php?id=wishing_hall:miao");
		}
//	}
}
...
if($_G['inajax']){
	...
	include template('wishing_hall:miao');
}
?>