...
$sql_rank="SELECT m.username,q.reward,q.uid FROM ".DB::table('wishing_hall_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY reward desc LIMIT 0,5";	
...
*/			
			$sql = "SELECT q.time,q.qdxq,q.uid,q.todaysay,q.godsay,m.username FROM ".DB::table('wishing_hall_wish_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY q.time desc LIMIT $start_limit, 10";
/*		}
