...
*************************/
	$random_rewards=(3/2)*($credit)*mt_rand(0,100)/100+mt_rand(0,100)/100;
		...
		DB::query("UPDATE ".DB::table('wishing_hall')." SET days=days+1,mdays=mdays+1,time='$_G[timestamp]',qdxq='$_GET[qdxq]',todaysay='$todaysay',godsay='$fastreplytexts[$godword]',flow_up=flow_up+{$credit},flow_award=flow_award+{$random_rewards},lastreward='$credit',lasted='1' WHERE uid='$_G[uid]'");
...
DB::query("INSERT INTO ".DB::table('wishing_hall_wish_offer')." (uid,time,qdxq,todaysay,godsay) VALUES ('$_G[uid]',$_G[timestamp],'$_GET[qdxq]','$todaysay','$fastreplytexts[$godword]')");
	$credit_munt=($random_rewards-$credit)*1024*1024*1024;
...
*************************/		
		if($exacr && $exacz) {
			...
			sign_msg("{$lang[tsn_14]}{$lang[tsn_03]}{$lang[tsn_04]}{$psc}{$lang[tsn_15]}{$lang[tsn_06]} 上传量 {$credit} GB  {$_G[setting][extcredits][$var[nrcredit]][unit]} {$lang[tsn_16]} 上传量 {$random_rewards} GB {$_G[setting][extcredits][$exacr][title]} {$exacz} {$_G[setting][extcredits][$exacr][unit]}.".$another_vip,"plugin.php?id=wishing_hall:offer");
		} else {
			DB::query("UPDATE ".DB::table('wishing_hall')." SET coin_award=coin_award+1 WHERE uid='$_G[uid]'");
			sign_msg("进贡上传量 {$credit} GB {$lang[tsn_18]} 上传量 {$random_rewards} GB .".$another_vip,"plugin.php?id=wishing_hall:offer");
		}
//	}
}
