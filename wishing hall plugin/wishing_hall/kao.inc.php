...
$qiandaodb_in_kao = DB::fetch_first("SELECT * FROM ".DB::table('wishing_hall_kao')." WHERE uid='$_G[uid]'");
$stats = DB::fetch_first("SELECT * FROM ".DB::table('wishing_hallset')." WHERE id='3'");
...
$emots = unserialize($_G['setting']['paulsign_emot_kao']);
...
$firstvister = DB::fetch_first("SELECT m.username,q.todaysay,q.godsay FROM ".DB::table('wishing_hall_wish_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid and time >= {$tdtime} ORDER BY q.time");
	if($firstvister){
		$firstvister="<li><span class='xi2 xg1'>".$firstvister['username']."</li><li style=' text-align:left;'><span style='color:#F00;'>心愿：</span>".$firstvister['todaysay']."</li><li style=' text-align:left;'><span style='color:#F00;'>考神说：</span>".$firstvister['godsay']."</li>";	
	}else{
...
$goodpeople = DB::fetch_first("SELECT m.username,q.todaysay,q.godsay FROM ".DB::table('wishing_hall_wish_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid and qdxq = 'shuai_kao' ORDER BY q.time desc");
	if($goodpeople){
		$goodpeople="<li><span class='xi2 xg1'>".$goodpeople['username']."</li><li style=' text-align:left;'><span style='color:#F00;'>心愿：</span>".$goodpeople['todaysay']."</li><li style=' text-align:left;'><span style='color:#F00;'>考神说：</span>".$goodpeople['godsay']."</li>";	
	}else{
	 	$goodpeople="<li><span class='xi2 xg1'>本庙久经风雨，几近坍塌。</li>";	
	}
		
$master=DB::fetch_first("SELECT m.username,q.todaysay,q.godsay FROM ".DB::table('wishing_hall_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY reward desc");	
	if($master){
		$master="<li><span class='xi2 xg1'>".$master['username']."</li><li style=' text-align:left;'><span style='color:#F00;'>心愿：</span>".$master['todaysay']."</li><li style=' text-align:left;'><span style='color:#F00;'>考神说：</span>".$master['godsay']."</li>";	
	}else{
	 	$master="<li><span class='xi2 xg1'>本庙暂无堂主</li>";	
	}
	
$sql_rank="SELECT m.username,q.reward,q.uid FROM ".DB::table('wishing_hall_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY reward desc LIMIT 0,8";	
...
***********6 E*********/		
//		$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('wishing_hall_kao')."");
//		$page = max(1, intval($_GET['page']));
//		$start_limit = ($page - 1) * 10;
//		$multipage = multi($num, 10, $page, "plugin.php?id=wishing_hall:kao&operation={$_GET[operation]}");
//	}
/*****5 S*******
...
*/			
			$sql = "SELECT q.time,q.qdxq,q.uid,q.todaysay,q.godsay,m.username FROM ".DB::table('wishing_hall_wish_kao')." q, ".DB::table('common_member')." m WHERE q.uid=m.uid ORDER BY q.time desc LIMIT 0, 10";
/*		}
...
*******5 E******/
	...
	while($mrc = DB::fetch($query)) {
		...
		if($mrc['qdxq']=="kx_kao"){$mrc['qdxq']="虔诚的想上自习，并说";
		}elseif($mrc['qdxq']=="ng_kao"){$mrc['qdxq']="小声的对神说想要与美女同桌，并说";
		}elseif($mrc['qdxq']=="ym_kao"){$mrc['qdxq']="想找人给占个座位，并说";
		}elseif($mrc['qdxq']=="wl_kao"){$mrc['qdxq']="想要一份考试真题，并说";
		}elseif($mrc['qdxq']=="nu_kao"){$mrc['qdxq']="求考神保佑一定要及格，并说";
		}elseif($mrc['qdxq']=="ch_kao"){$mrc['qdxq']="向神祈求一个免挂金牌，并说";
		}elseif($mrc['qdxq']=="yl_kao"){$mrc['qdxq']="请求速速打通考霸模式，并说";
		}elseif($mrc['qdxq']=="shuai_kao"){$mrc['qdxq']="要给考神修庙建祠堂，并说";
		}
		$mrcs[] = $mrc;
		$has_items=1;
	}
	...
	if($_G['adminid'] == 1) {
		DB::query("UPDATE ".DB::table('wishing_hall_wish_kao')." SET todaysay='{$lang['ban_01']}' WHERE uid='".intval($_GET['banuid'])."'");
		DB::query("UPDATE ".DB::table('wishing_hall_kao')." SET todaysay='{$lang['ban_01']}' WHERE uid='".intval($_GET['banuid'])."'");
...
/*************3 E******************/	
	...
	if($var['lockopen']){
		while(discuz_process::islocked('wishing_hall_kao', 5)){
			usleep(100000);
		}
	}
...
*/	
	...
	$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('wishing_hall_wish_kao')." WHERE time >= {$tdtime}");
	if(!$qiandaodb['uid']) {
...
*********************************/	
	$credit=DB::result_first("SELECT price FROM ".DB::table('wishing_hallemot')." WHERE qdxq='$_GET[qdxq]' and god_id=3");
...
*************************/	
	$random_rewards=ceil((($credit/15)*mt_rand(0,100)/100+mt_rand(0,100)/100)*2);
	updatemembercount($_G['uid'], array( "extcredits5"=> $random_rewards));
		
		DB::query("UPDATE ".DB::table('wishing_hall')." SET days=days+1,mdays=mdays+1,time='$_G[timestamp]',qdxq='$_GET[qdxq]',todaysay='$todaysay',godsay='$fastreplytexts[$godword]',reward=reward+{$credit},chip_award=chip_award+{$random_rewards},lastreward='$credit',lasted='1' WHERE uid='$_G[uid]'");
DB::query("UPDATE ".DB::table('wishing_hall_kao')." SET todaysay='$todaysay',godsay='$fastreplytexts[$godword]',reward=reward+{$credit} WHERE uid='$_G[uid]'");		
//	}
/*************2 S******************/
DB::query("INSERT INTO ".DB::table('wishing_hall_wish_kao')." (uid,time,qdxq,todaysay,godsay) VALUES ('$_G[uid]',$_G[timestamp],'$_GET[qdxq]','$todaysay','$fastreplytexts[$godword]')");
	$credit_munt=-$credit;
...
/*************2 E******************/	
	...
	if($num <= ($extreward_num - 1) ) {
		...
		if($var['qdtype'] == '2') {
			...
			if($num=='0' || $stats['qdtidnumber'] == '0') {
				$subject=str_replace(array('{m}','{d}','{y}','{bbname}','{author}','{godname}'),array(dgmdate($_G['timestamp'], 'n',$var['tos']),dgmdate($_G['timestamp'], 'j',$var['tos']),dgmdate($_G['timestamp'], 'Y',$var['tos']),$_G['setting']['bbname'],$_G['username'],"考神堂"),$var['title_thread']);
				$hft = dgmdate($_G['timestamp'], 'Y-m-d H:i',$var['tos']);
				if($exacr && $exacz) {
					$message = "[quote][size=2][color=dimgray]{$lang[tsn_10]}[/color][url={$_G[siteurl]}plugin.php?id=wishing_hall:kao][color=darkorange]{$lang[tsn_11]}[/color][/url][color=dimgray]{$lang[tsn_12]}[/color][/size][/quote][quote][size=2][color=gray][color=teal] [/color][color=gray]{$lang[tsn_01]}[/color] [color=darkorange]{$hft}[/color] {$lang[tsn_02]}[color=red]{$lang[tsn_03]}[/color][color=darkorange]{$lang[tsn_04]}{$lang[tsn_13]}{$lang[tsn_05]}[/color]{$lang[tsn_06]} [/color][color=gray]{$_G[setting][extcredits][$var[nrcredit]][title]} [/color][color=darkorange]{$credit}[/color][color=gray]{$_G[setting][extcredits][$var[nrcredit]][unit]}[/color][color=gray]{$lang[tsn_17]}[/color] [color=gray]{$_G[setting][extcredits][$exacr][title]} [/color][color=darkorange]{$exacz}[/color][color=gray]{$_G[setting][extcredits][$exacr][unit]}.{$another_vip}[/color][/color][/size][/quote][size=3][color=dimgray]{$lang[tsn_07]}[color=red]{$todaysay}[/color]{$lang[tsn_08]}[/color][/size]";
				} else {
				...
				$tid = DB::insert_id();
				DB::query("UPDATE ".DB::table('wishing_hallset')." SET qdtidnumber = '$tid' WHERE id='3'");
	...
	if($num ==0) {
		if($stats['todayq'] > $stats['highestq']) DB::query("UPDATE ".DB::table('wishing_hallset')." SET highestq='$stats[todayq]' WHERE id='3'");
		DB::query("UPDATE ".DB::table('wishing_hallset')." SET yesterdayq='$stats[todayq]',todayq=1 WHERE id='3'");
		DB::query("UPDATE ".DB::table('wishing_hallemot')." SET count=0 WHERE god_id='3'");
		DB::query("DELETE from ".DB::table('wishing_hall_wish_kao')." where time<{$tdtime}-3600*24");//删除前天及以前的记录
	} else {
		DB::query("UPDATE ".DB::table('wishing_hallset')." SET todayq=todayq+1 WHERE id='3'");
	}
	...
	if($var['lockopen']) discuz_process::unlock('wishing_hall_kao');
/************************	
	if($var['tzopen']) {
		if($exacr && $exacz) {
...
*************************/		
		if($exacr && $exacz) {
			...
			sign_msg("{$lang[tsn_14]}{$lang[tsn_03]}{$lang[tsn_04]}{$psc}{$lang[tsn_15]}{$lang[tsn_06]} 金币 {$credit} {$_G[setting][extcredits][$var[nrcredit]][unit]} {$lang[tsn_16]} 筹码 {$random_rewards} {$_G[setting][extcredits][$exacr][title]} {$exacz} {$_G[setting][extcredits][$exacr][unit]}.".$another_vip,"plugin.php?id=wishing_hall:kao");
		} else {
			DB::query("UPDATE ".DB::table('wishing_hall')." SET coin_award=coin_award+1 WHERE uid='$_G[uid]'");
			sign_msg("进贡金币 {$credit} {$lang[tsn_18]} 筹码 {$random_rewards} .".$another_vip,"plugin.php?id=wishing_hall:kao");
		}
//	}
}
